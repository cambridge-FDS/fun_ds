function [val, policies] = valueold7(a,params, pi0, pi1, rho, Ach, state, V0_guess)
pi0=pi0(3); pi1=pi1(3);
...
elseif l+t+p>=1
    ...
    if r==1 %unemployed
        val=params.p_exp*((q_e*c)^(1-params.sigma))/(1-params.sigma)+(1-params.p_exp)*(c^(1-params.sigma))/(1-params.sigma) -...
            params.psi* ((1-l)^(1+params.eta)/(1+params.eta)) + params.beta*(1-delta_o)* pi_after* params.Vheaven +...
            params.beta*delta_o*gg'*[pi0-params.cs_effect*phi_r; 1-(pi0-params.cs_effect*phi_r)];
    else
        val=params.p_exp*((q_e*c)^(1-params.sigma))/(1-params.sigma)+(1-params.p_exp)*(c^(1-params.sigma))/(1-params.sigma) -...
            params.psi* ((1-l)^(1+params.eta)/(1+params.eta)) + params.beta*(1-delta_o)* pi_after* params.Vheaven +...
            params.beta*delta_o*gg'*[1-pi1-params.cs_effect*phi_r; pi1+params.cs_effect*phi_r];
    end
end
...
policies = [a(1) d l p t c pi0-params.cs_effect*phi_r pi1+params.cs_effect*phi_r params.w*rho*params.eps0o*z0 (params.beta*(1-delta_o)* pi_after* params.Vheaven)/(1-params.beta*delta_o) 0];
val=-val;