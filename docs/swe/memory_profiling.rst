...
Memray is particularly useful because it tracks allocations at a low level and can generate rich reports such as **flame graphs** and **tables**. :contentReference[oaicite:0]{index=0}
...
Memray provides a CLI called ``memray`` and can also be invoked as a Python module. :contentReference[oaicite:1]{index=1}
...
The manpages summarize it like this: run ``memray run`` then use a reporter like ``flamegraph`` or ``table``. :contentReference[oaicite:2]{index=2}
...
This creates a binary capture file that reporters can analyze. :contentReference[oaicite:3]{index=3}
...
This generates an HTML flame graph you can open in a browser. :contentReference[oaicite:4]{index=4}
...
* you can inspect hotspots by drilling into the largest frames :contentReference[oaicite:5]{index=5}

Table report (terminal or file)
-------------------------------

Memray also provides a ``table`` reporter to get a more compact overview. The CLI supports multiple reporter subcommands (e.g., ``table``, ``tree``, ``summary``). :contentReference[oaicite:6]{index=6}
...
It uses internal allocators and memory pools for many small objects, and only sometimes calls ``malloc/free`` underneath. Memray’s documentation explains these Python allocator layers and why they matter when interpreting profiles. :contentReference[oaicite:7]{index=7}
...
.. note::
   Memray’s flamegraph documentation also describes a **temporal flame graph** mode, which visualizes memory usage over time and lets you explore time windows. :contentReference[oaicite:8]{index=8}
...
This runs tests with Memray enabled and prints a memory report per test suite run. :contentReference[oaicite:9]{index=9}

Leak-focused testing
--------------------

pytest-memray can analyze allocations made during a test that are not freed by the end of the test body and fail if a threshold is exceeded. :contentReference[oaicite:10]{index=10}
...
That may be normal due to Python’s allocator pooling of small objects. Understanding Python allocators helps interpret results correctly. :contentReference[oaicite:11]{index=11}
...
* Memray documentation (overview): :contentReference[oaicite:12]{index=12}
* Getting started guide: :contentReference[oaicite:13]{index=13}
* Flame graph reporter docs (including temporal mode): :contentReference[oaicite:14]{index=14}
* Memray README (CLI examples): :contentReference[oaicite:15]{index=15}
* pytest-memray docs (usage + leak analysis): :contentReference[oaicite:16]{index=16}
